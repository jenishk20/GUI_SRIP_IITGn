...
from pya import Polygon,Vector,Point,Edge,Region,Box
  ...
  def __init__(self, polygon, min_length):
    ...
    for c in range(0, contours):
      ...
      for e in polygon.each_edge(c):
        #Printing Edge Coordinates
        #print("Egde Coordinate:",e)
        #print("Edge Length:",e.length())
        #print("Edge param:",e.d())
        ...
        if e.length() < min_length * 2:
          ...
          #nmin specifies the number of fragments the edge will be divided into
          nmin = (e.length() // min_length) - 2
          
          norm = 1.0 / e.length()
          
          #rem specifies the remainder length except from fragmentation
          ...
          second_point = first_point + e.d() * (rem * norm)
        
          fragments.append(Fragment(Edge(first_point, second_point)))
          #print("\n")
          first_point = second_point
          for i in range(0, nmin):
            #print(e.d()*(min_length*norm))
            second_point= first_point + e.d() * (min_length * norm)
            #print(pp)
            fragments.append(Fragment(Edge(first_point, second_point)))
            first_point = second_point

          fragments.append(Fragment(Edge(first_point, e.p2)))
...
class LayoutViewer2(tkinter.Frame):
    ...
    def __init__(
        ...
        self.zommm = tkinter.Button(self.frame, text="Next Slide", command=self._destroy)
        ...
        self.zommm.grid(row=0, column=6, sticky="w")
#         self.bool.grid(row=1,column=4,sticky="w")
...
#         self.canvas.delete('all')
    ...
    #Function to allow user to Browse Files from Local Disk
...
#         lib3=gdspy.GdsLibrary()
#         cell=lib3.new_cell('L')
#         cell.add(gdspy.boolean(pol_dict[2,0],pol_dict[4,0],'not'))
#         lib3.write_gds('out.gds')
...
# #         self._load_in()
    ...
    #Function for Boolean Operation Module
    
    def bool_operation_module(layer1,layer2,filename,ope):
        
        layer_1_num=layer1[0]
        layer_1_dt=layer1[2]
        
        layer_2_num=layer2[0]
        layer_2_dt=layer2[2]
        
        
        print(layer_1_num,layer_1_dt,layer_2_num,layer_2_dt,ope)
        ...
        flag=False
        flag1=False
        for ly_id in ly.layer_indices():
            res=ly.get_info(ly_id).to_s()
            
            
            if res[0]==layer_1_num and res[2]==layer_1_dt:
                flag=True
            if res[0]==layer_2_num and res[2]==layer_2_dt:
                flag1=True
        
        if(flag==False or flag1==False):
            return False
        
        layer_1_num=int(layer_1_num)
        layer_1_dt=int(layer_1_dt)
        layer_2_num=int(layer_2_num)
        layer_2_dt=int(layer_2_dt)
        
        l1 = ly.layer(layer_1_num, layer_1_dt)
        l2 = ly.layer(layer_2_num, layer_2_dt)
        
        r1 = pya.Region(ly.top_cell().begin_shapes_rec(l1))
        r2 = pya.Region(ly.top_cell().begin_shapes_rec(l2))
        
        top = layout.create_cell("TOP")
        l10 = layout.layer(9, 0)
        ll2=layout.layer(2,0)
        ll3=layout.layer(4,0)
        ...
        top.shapes(ll2).insert(r1)
        top.shapes(ll3).insert(r2)
        top.shapes(l10).insert(boolean_ope)
        
        opfile="bool_output.gds"
        layout.write(opfile)
        
        return True
    
    #Function to provide Fragmentation functionality to user
    
    def fragmentation_module(s,layer,datatype,filename):
        
        res=s.isnumeric()
        res1=layer.isnumeric()
        res2=datatype.isnumeric()
        if(res==False or res1==False or res2==False):
            return False
       
       
        layout_data = pya.Layout()
        layout = pya.Layout()
        layout_data.read(filename)
#         print(s,layer,datatype,filename)
        flag=False
        for ly_id in layout_data.layer_indices():
            res=layout_data.get_info(ly_id).to_s()
            
#             print(res)
            res=res.split('/')
            if res[0]==layer and res[1]==datatype:
                flag=True
...
#         print(flag)
        ...
        layer4 = layout_data.layer(layer, datatype)
        polygon_shapes = layout_data.top_cell().begin_shapes_rec(layer4)
        top = layout.create_cell("TOP")
        l10 = layout.layer(10, 0)
        l2=layout.layer(2,0)
        top.shapes(l2).insert(polygon_shapes)
        while not polygon_shapes.at_end():

          shape_var = polygon_shapes.shape()
          if shape_var.is_polygon():

                polygon_for_fragmentation = polygon_shapes.itrans() * shape_var.polygon

                poly_in = Polygon(polygon_for_fragmentation)
#                 print("Coordinates of Shape:", poly_in)
#                 print('\n')

                fragments = Fragments(poly_in, 300)
#                 print("Fragments Details:", fragments)
                ...
                top.shapes(l10).insert(poly)
          polygon_shapes.next()
        
        layout.write("fggs.gds")
        return True
...
def browseFiles():
    ...
    sub_btn['text']="Do Fragmentation"
    sub_btn1['text']="Do Boolean Operation"
    label_file_explorer.configure(text="File Opened: "+filename)
    
    viewer(filename)


#Calling Fragmentation Function from Layout Viewer Class

def submit():
    
    val=name_var.get()
    layer_info=layer_name.get().split(',')
#     datatype=layer_datatype.get()
    filename=file_path_var.get()
    layer=layer_info[0]
    datatype=layer_info[1]
    print(layer,datatype)
    if(LayoutViewer2.fragmentation_module(val,layer,datatype,filename))==True:
        sub_btn['text']='Success, Output Saved to file named "fggs.gds"'
    
    else:
        sub_btn['text']='Failure,Enter Correct Value'
        
    name_var.set("")
    layer_name.set("")
    
    
# Calling Boolean Operations Function from Layout Viewer Class

def submit1():
    
    layer_1=layer_name_1.get()
    layer_2=layer_name_2.get()
    filename=file_path_var.get()
    ope=n.get()
    
    print(layer_1,layer_2,filename,ope)
    
    #Calling Function from LayoutViewer Class
    if(LayoutViewer2.bool_operation_module(layer_1,layer_2,filename,ope))==True:
        sub_btn1['text']='Success,Output Saved to "bool_output.gds"'
    else:
        sub_btn1['text']='Failure'
        
    n.set("AND")
    layer_name_1.set("2,0")
    layer_name_2.set("4,0")

#Help Documentation Redirect
def callback():
    webbrowser.open_new(r"C:\Users\Owner\Desktop\Helper\Readme.md")
...
# layer_datatype=tk.StringVar()
file_path_var=tk.StringVar()

#Layer Names for Boolean Operations
layer_name_1=tk.StringVar()
layer_name_2=tk.StringVar()
...
label_file_explorer = Label(window,
                            text = "Standalone Layout Viewer GUI",
                            width = 50, height = 2,
                            fg = "blue")

label_frag = Label(window,
                            text = "Fragmentation ",
                            width = 20, height = 1,
                            fg = "blue")
label_bool = Label(window,
                            text = "Boolean Operations",
                            width = 20, height = 1,
                            fg = "blue")
  
name_label = Label(window, text = 'Enter Fragmentation Size', font=('calibre',10, 'bold'))
name_entry = Entry(window,textvariable = name_var, font=('calibre',10,'normal'),width=10)


layer_label = Label(window, text = 'Enter Layer Info', font=('calibre',10, 'bold'))
layer_entry = Entry(window,textvariable = layer_name, font=('calibre',10,'normal'),width=10)
layer_entry.insert(0,'2,0')

# layer_datatype_label = Label(window, text = 'Enter Layer DataType', font=('calibre',10, 'bold'))
# layer_datatype_entry = Entry(window,textvariable = layer_datatype, font=('calibre',10,'normal'),width=10)

sub_btn=Button(window,text = 'Do Fragmentation', command = submit)
sub_btn1=Button(window,text = 'Do Boolean Operation',command=submit1)

layer_1_label=Label(window,text='Enter layer 1 Info',font=('calibre',10,'bold'))
layer_1_entry=Entry(window,textvariable=layer_name_1,font=('calibre',10),width=10)
layer_1_entry.insert(0,'2,0')

layer_2_label=Label(window,text='Enter layer 2 Info',font=('calibre',10,'bold'))
layer_2_entry=Entry(window,textvariable=layer_name_2,font=('calibre',10),width=10)
layer_2_entry.insert(0,'4,0')
...
button_help=Button(window,text='Help',command=callback)

ttk.Label(window, text = "Select the Boolean Operation:", 
        font = ("calibre", 10,'bold')).grid(column = 1, 
        row = 10, padx = 100, pady = 25)


n = tk.StringVar()
monthchoosen = ttk.Combobox(window, width = 5, 
                            textvariable = n)


monthchoosen['values'] = ('XOR', 
                          'OR',
                          'AND',
                          'DIFF')
...
monthchoosen.grid(column = 2, row = 10)


name_label.grid(column=1,row=4)
name_entry.grid(column=2,row=4)

layer_label.grid(column=1,row=5)
layer_entry.grid(column=2,row=5)

# layer_datatype_label.grid(column=1,row=4)
# layer_datatype_entry.grid(column=2,row=4)

sub_btn.grid(column=3,row=5)
sub_btn1.grid(column=3,row=10)


layer_1_label.grid(column=1,row=8)
layer_1_entry.grid(column=2,row=8)

layer_2_label.grid(column=1,row=9)
layer_2_entry.grid(column=2,row=9)

label_file_explorer.grid(column = 1, row = 1)
label_frag.grid(column=2,row=3)
label_bool.grid(column=2,row=6)
  
button_explore.grid(column = 2, row = 1)
button_exit.grid(column = 3,row = 1)
button_help.grid(column=4,row=1)
